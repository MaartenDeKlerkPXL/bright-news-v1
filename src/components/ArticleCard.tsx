...
export const ArticleCard: React.FC<ArticleCardProps> = ({article, onReadMore, isSaved, onToggleSave, language}) => {
    ...
    return (
        <div onClick={() => onReadMore(article.id)}
             className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-green-50 flex flex-col h-full cursor-pointer">
            <div className="relative h-48 overflow-hidden">
                <ImageWithFallback src={article.image} alt={article.title}
                                   className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"/>
                <div className="absolute top-3 right-3 flex gap-2">
                    <span
                        className="px-3 py-1 bg-white/95 backdrop-blur-sm text-green-700 rounded-full text-xs">{getCatTranslation(article.category)}</span>
                    <button onClick={(e) => {
                        e.stopPropagation();
                        onToggleSave(article.id);
                    }}
                            className="w-8 h-8 bg-white/95 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-green-100 transition-colors"
                            aria-label={isSaved ? t(language, 'unsaveArticle') : t(language, 'saveArticle')}>
                        {isSaved ? <BookmarkCheck className="w-4 h-4 text-green-600 fill-green-600"/> :
