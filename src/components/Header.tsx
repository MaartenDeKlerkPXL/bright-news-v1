...
import { Bookmark, User, Home } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="flex items-center gap-2">
            <button onClick={() => onNavigate('home')} className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl transition-all ${currentPage === 'home' ? 'bg-green-100 text-green-800' : 'text-gray-600 hover:bg-green-50'}`}>
              <Home className="w-5 h-5" />
              <span className="hidden sm:inline">{t(language, 'home')}</span>
            </button>
            
            <button onClick={() => onNavigate('saved')} className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl transition-all ${currentPage === 'saved' ? 'bg-green-100 text-green-800' : 'text-gray-600 hover:bg-green-50'}`}>
              <Bookmark className="w-5 h-5" />
              <span className="hidden sm:inline">{t(language, 'saved')}</span>
            </button>
            
            <button onClick={() => onNavigate('profile')} className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl transition-all ${currentPage === 'profile' ? 'bg-green-100 text-green-800' : 'text-gray-600 hover:bg-green-50'}`}>
