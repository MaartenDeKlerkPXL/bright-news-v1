...
import { ArrowLeft, Bookmark, BookmarkCheck, Share2, Calendar, MapPin, Tag } from 'lucide-react';
...
export const ArticleDetailPage: React.FC<ArticleDetailPageProps> = ({ articleId, onBack, isSaved, onToggleSave, subscriptionPlan, language, savedArticles, onNavigateToArticle, onNavigateToProfile }) => {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={onBack} className="text-green-600 hover:text-green-700">{t(language, 'backToHome')}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-600 hover:text-green-600 mb-6 transition-colors group">
        ...
        <article className="bg-white rounded-3xl overflow-hidden shadow-lg border border-green-100 mb-12">
          <div className="relative h-[28rem]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8">
              <div className="flex items-center gap-3 mb-4">
                <span className="inline-flex items-center gap-2 px-4 py-2 bg-white/95 backdrop-blur-sm text-green-700 rounded-full text-sm font-medium">
                  <Tag className="w-4 h-4" />
                  {getCatTranslation(article.category)}
                </span>
                <span className="inline-flex items-center gap-2 px-4 py-2 bg-white/95 backdrop-blur-sm text-gray-700 rounded-full text-sm">
          ...
          <div className="p-8 lg:p-12">
            ...
            <div className="flex items-center gap-3 mb-8 pb-8 border-b border-gray-100">
              <button onClick={() => onToggleSave(article.id)} className={`flex items-center gap-2 px-6 py-3 rounded-full transition-all font-medium ${isSaved ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {isSaved ? <BookmarkCheck className="w-5 h-5" /> : <Bookmark className="w-5 h-5" />}
                <span>{isSaved ? t(language, 'unsaveArticle') : t(language, 'saveArticle')}</span>
              </button>
              <button onClick={handleShare} className="flex items-center gap-2 px-6 py-3 bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition-all font-medium">
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedArticles.map(a => (
                <ArticleCard key={a.id} article={a} onReadMore={onNavigateToArticle} isSaved={savedArticles.includes(a.id)} onToggleSave={onToggleSave} language={language} />
              ))}
            </div>
          </div>
        )}
