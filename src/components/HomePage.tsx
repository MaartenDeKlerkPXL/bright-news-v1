...
export const HomePage: React.FC<HomePageProps> = ({onNavigateToArticle, savedArticles, onToggleSave, language}) => {
    ...
    const getCatTranslation = (cat: string) => {
        const map: Record<string, any> = {
            'All': 'allCategories', 'Technology': 'technology', 'Green World': 'greenWorld',
    ...
    const getRegTranslation = (reg: string) => {
        const map: Record<string, any> = {
            'All Regions': 'allRegions', 'Europe': 'europe', 'Asia': 'asia',
        ...
        <div className="min-h-screen bg-gradient-to-b from-green-50 to-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-green-100 mb-6">
                    <div className="flex items-center justify-between p-5 border-b border-gray-100">
                        <button onClick={() => setIsFilterExpanded(!isFilterExpanded)}
                            ...
                            {hasActiveFilters && (
                                <span
                                    className="ml-2 px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full">{t(language, 'active')}</span>
                            )}
                            {isFilterExpanded ? <ChevronUp className="w-5 h-5 text-gray-500 ml-2"/> :
                                <ChevronDown className="w-5 h-5 text-gray-500 ml-2"/>}
                        </button>
                        {hasActiveFilters && (
                            <button onClick={clearFilters}
                                    className="px-3 py-1 text-xs text-gray-600 hover:text-green-600 flex items-center gap-1 transition-colors">
                                <X className="w-3 h-3"/>
                                {t(language, 'cancel')}
                            </button>
                        )}
                                ...
                                <div className="flex flex-wrap gap-2 justify-center">
                                    {categories.map(cat => (
                                        <button key={cat} onClick={() => toggleCategory(cat)}
                                                className={`px-4 py-2 rounded-full text-sm transition-all ${selectedCategories.includes(cat) ? 'bg-green-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                ...
                                <div className="flex flex-wrap gap-2 justify-center">
                                    {regions.map(reg => (
                                        <button key={reg} onClick={() => toggleRegion(reg)}
                                                className={`px-4 py-2 rounded-full text-sm transition-all ${selectedRegions.includes(reg) ? 'bg-green-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                ...
                <div className="mb-6 flex items-center justify-between">
                    <p className="text-gray-600 text-sm">{t(language, 'showing')} <span
                        className="text-green-700 font-medium">{filteredArticles.length}</span> {t(language, 'positive')} {filteredArticles.length === 1 ? t(language, 'story') : t(language, 'stories')}
                    </p>
                    {hasActiveFilters && (
                ...
                {filteredArticles.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredArticles.map(article => (
                            <ArticleCard key={article.id} article={article} onReadMore={onNavigateToArticle}
                                         isSaved={savedArticles.includes(article.id)} onToggleSave={onToggleSave}
                                         language={language}/>
                        ))}
                    </div>
                ) : (
